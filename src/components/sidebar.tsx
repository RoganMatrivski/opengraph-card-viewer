    ...
    >
      ...
      {urls.length === 0 ? (
        ...
        <ul className="space-y-2">
          {urls.map((item) => (
            <li key={item.url} className="flex items-center justify-between gap-2 rounded-md border bg-background p-2">
              <span className="text-sm truncate" title={item.label}>
                {item.label}
              </span>
              <Button variant="ghost" size="icon" className="h-6 w-6 shrink-0" onClick={() => removeUrl(item.url)}>
                <X className="h-4 w-4" />
